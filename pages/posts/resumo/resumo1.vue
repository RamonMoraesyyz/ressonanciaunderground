<template>
    <div class="container mx-auto pt-24 pb-8 px-4 sm:px-6 lg:px-8">
      <!-- Ajuste de padding para telas menores -->
      <article class="max-w-3xl mx-auto bg-black text-white shadow-md rounded-lg p-6 sm:p-8">
        <!-- Título do post -->
        <h1 class="text-3xl sm:text-4xl font-bold mb-2">Resumo Semanal 1</h1> <!-- Ajuste de tamanho do texto para responsividade -->
  
        <!-- Data e autor -->
        <p class="text-sm text-gray-500 mb-4">Publicado em 25 de agosto de 2024 por Ramon Moraes</p>
  
        <!-- Espaço para a foto -->
        <div class="mb-6">
          <img src="/public/resumo/resumo1.jfif" alt="Isac Varzim" class="w-full h-auto rounded-lg">
        </div>
        <!-- Conteúdo do post -->
        <div class="space-y-6">
          
          <!-- Terraplana -->
          <div>
            <p class="text-xl font-bold mb-1">Mountaineer</p>
            <p class="text-white text-base mb-1">Um post-metal americano com grandes influências de doom metal que é difícil explicar.</p>
            <a href="https://open.spotify.com/intl-pt/artist/6Elws371EaSX8AiIMeIS9q?si=ZlPh9jqiQvu7WdjG39pBpQ&nd=1&dlsi=da567470a4304371" target="_blank" class="text-green-400 font-bold hover:underline">Spotify</a>
          </div>
          
          <!-- Gorduratrans -->
          <div>
            <p class="text-xl font-bold mb-1">Black Bordello</p>
            <p class="text-white text-base mb-1">Indie passivo/agressivo inglês.</p>
            <a href="https://open.spotify.com/intl-pt/artist/0LhmtJ38vRhd35Eg3oC1nt" target="_blank" class="text-green-400 font-bold hover:underline">Spotify</a>
          </div>
          
          <!-- The Jesus and Mary Chain -->
          <div>
            <p class="text-xl font-bold mb-1">Oceans of Slumber</p>
            <p class="text-white text-base mb-1">Um metal progressivo e épico com um vocal feminino lá do Texas..</p>
            <a href="https://open.spotify.com/intl-pt/artist/2JSza6IRxLr1Ez3wqKd0SY" target="_blank" class="text-green-400 font-bold hover:underline">Spotify</a>
          </div>


          <div>
            <p class="text-xl font-bold mb-1">Escuela Grind</p>
            <p class="text-white text-base mb-1">Metal extremo com vocal feminino americano que é de tirar o fôlego.</p>
            <a href="https://open.spotify.com/intl-pt/artist/75lXMXjUfNNbrBz6auGvvG" target="_blank" class="text-green-400 font-bold hover:underline">Spotify</a>
          </div>
          
        </div>
    
        <!-- Botão para voltar à página inicial -->
        <div class="flex justify-center mt-8">
          <NuxtLink to="/" class="px-4 py-2 bg-gray-800 text-white rounded hover:bg-gray-700">Voltar para a página inicial</NuxtLink>
        </div>
      </article>
    </div>
  </template>
  
  <script setup>
    // Adicione qualquer script necessário aqui
  </script>
  
  <style scoped>
  /* Estilos adicionais podem ser adicionados aqui se necessário */
  </style>
  